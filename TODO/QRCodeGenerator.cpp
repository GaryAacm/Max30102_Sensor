...
std::string QRCodeGenerator::generateAndSendUserMessage()
{
    ...
    std::string sample_id = device_serial + "-" + current_time + "-" + numbers_and_letters;
    ...
    // Save QR code as PNG
    if (!saveQrPng("QRcode.png", qrcode))
    {
    ...
    // Print sample_id and user_uuid (if any)
